...
#    username = StringField('username', validators=[validators.DataRequired(), validators.Length(min=4, max=15)])
#    password = PasswordField('password', validators=[validators.DataRequired(), validators.Length(min=8, max=80)])
#    remember = BooleanField('remember me')
#
#class RegisterForm(FlaskForm):
#    email = StringField('email', validators=[validators.DataRequired(), validators.Email(message='Invalid email'), validators.Length(max=50)])
#    username = StringField('username', validators=[validators.DataRequired(), validators.Length(min=4, max=15)])
#    password = PasswordField('password', validators=[validators.DataRequired(), validators.Length(min=8, max=80)])
#    
#    
...
#        state = ("SELECT UID, PAN_Pass as dbpass , PAN_User , PAN_Email FROM PANUser WHERE PAN_User = '%s' ;")  %(username)
#        result = cur.execute(state)
#        results = cur.fetchone()
#        dbpassword = results['dbpass']
#        id = results['UID']
#        #print (dbpassword , form.password.data )
#        if check_password_hash(dbpassword, form.password.data):
#            user = User(id)
#            login_user(user)
#            session['UID'] = results['UID']
#            session['name'] = results['PAN_User']
#            session['email'] = results['PAN_Email']
...
#        ## cursor ##
#        cur = mysql.connection.cursor()
#        cur.execute(" INSERT INTO `PANUser` (`PAN_User`, `PAN_Pass`,`PAN_Email`)VALUES ( %s,%s,%s) ", (username , hashed_password, email) )
#        
#        ## commit and close ##
...
def setup():
    ...
    if request.method == 'POST' and form.validate():
        ...
        ipmankey = "ipman = '%s' \n\n" %(ipman)
        adminuserkey = "adminuser = '%s' \n\n" %(adminuser)
        adminpwdkey = "adminpwd = '%s' \n\n" %(adminpwd)
